...
module.exports = class Si1145 {
    ...
    initialize() {
        ...
        this.writeParam(this.param.CHLIST, (this.param.CHLIST_ENUV | 
            this.param.CHLIST_ENALSIR | this.param.CHLIST_ENALSVIS));

        // enable interrupt on every sample
        this.bus.writeByteSync(this.device.addr, this.register.INTCFG, this.register.INTCFG_INTOE);
        this.bus.writeByteSync(this.device.addr, this.register.IRQEN, this.register.IRQEN_ALSEVERYSAMPLE);
        ...
        this.writeParam(this.param.ALSVISADCMISC, this.param.ALSVISADCMISC_VISRANGE);

        // measurement rate for auto
        this.bus.writeByteSync(this.device.addr, this.register.MEASRATE0, 0xFF); // 255 * 31.25uS = 8ms
        
        // auto run
        this.bus.writeByteSync(this.device.addr, this.register.COMMAND, this.command.PSALS_AUTO);
    ...
    valueAtIndex(idx, callback) {
        if (!this.isIdxInRange(idx)) {
            ...
            this.bus.readI2cBlock(this.device.addr, readReg, 2, Buffer.alloc(2), (err, bytesRead, buffer) => {
                if (err) {
                    callback(err);
                }
                else {
                    ...
                    if (idx == 2) {
                        this.device.parameters[idx].value = Math.round(this.device.parameters[idx].value / 100);
                    }
    ...
    valueAtIndexSync(idx) {
        if (!this.isIdxInRange(idx)) {
            this.logError(`Si1145 Error: readValueAtIndexSync(${idx}) index out of range`);
            return NaN;
        }
        ...
        this.bus.readI2cBlockSync(this.device.addr, readReg, 2, buffer);
    ...
    async writeParam(p, v) {
        ...
        this.bus.writeByteSync(this.device.addr, this.register.COMMAND, 0x00);
        var response = this.bus.readByteSync(this.device.addr, this.register.RESPONSE);

        while (response == 0x00) {
            this.bus.writeByteSync(this.device.addr, this.register.PARAMWR, v);
            this.bus.writeByteSync(this.device.addr, this.register.COMMAND, p | this.command.PARAM_SET);
            await this.sleep(2);
            response = this.bus.readByteSync(this.device.addr, this.register.RESPONSE);
        }
    }

    async reset() {
        this.bus.writeByteSync(this.device.addr, this.register.MEASRATE0, 0);
        this.bus.writeByteSync(this.device.addr, this.register.MEASRATE1, 0);
        this.bus.writeByteSync(this.device.addr, this.register.IRQEN, 0);
        this.bus.writeByteSync(this.device.addr, this.register.IRQMODE1, 0);
        this.bus.writeByteSync(this.device.addr, this.register.IRQMODE2, 0);
        this.bus.writeByteSync(this.device.addr, this.register.INTCFG, 0);
        this.bus.writeByteSync(this.device.addr, this.register.IRQSTAT, 0xFF);
        
        this.bus.writeByteSync(this.device.addr, this.register.COMMAND, this.command.RESET);
        await this.sleep(10);
        this.bus.writeByteSync(this.device.addr, this.register.HWKEY, this.register.PARAMWR);
        await this.sleep(10);
    }
    ...
    setCalibration() {
        ...
        this.bus.writeByteSync(this.device.addr, this.register.UCOEFF0, 0x52);
        this.bus.writeByteSync(this.device.addr, this.register.UCOEFF1, 0x7A);
        this.bus.writeByteSync(this.device.addr, this.register.UCOEFF2, 0x02);
        this.bus.writeByteSync(this.device.addr, this.register.UCOEFF3, 0x00);
    }
